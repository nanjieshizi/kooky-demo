...
async function fetchBlobForFile(url) {
  ...
  if (isDeerflowThreadArtifactUrl(url)) {
    return threadApi.fetchThreadArtifactBlob(url)
  }
  try {
...
// 是否可下载：有 URL 或有内联内容（mock/library 文件）
function canDownload(file) {
  return hasUsableUrl(file) || !!file?.content
}

// 统一下载：优先走 URL 鉴权 blob，否则用内联 content 生成本地文件
function smartDownload(file) {
  if (hasUsableUrl(file)) {
  ...
  ElMessage?.info?.(`已下载「${file?.name || '文件'}」（演示）`)
}

// 转存到右侧工具栏「文件」全局库
function saveToLibrary(file) {
  const previewType = file?.previewType || getPreviewType(file) || 'other'
  const ok = fileLibraryStore.addFile({
    name: file?.name,
    type: previewType,
    previewType,
  ...
  ElMessage?.[ok ? 'success' : 'info']?.(
    ok ? `已转存「${file?.name}」到文件库` : `「${file?.name}」已在文件库中`,
  )
}
...
function openFilePreview(file) {
  // 左侧会话文件 → 会话侧区「文件预览」槽（多标签），不走独立面板/弹窗
  if (file?.fromLibrary || file?.mockPreview) {
    sidePanelStore.open('preview')
    previewStore.openFile({
      id: file.name,
      name: file.name,
      fileType: file.previewType || 'other',
      content: file.content || '',
  ...
  // 获取当前线程的 agentId
  const threadRow = deerflowStore.threads.find((t) => Number(t.id) === Number(props.spaceId))
  ...
  window.electronAPI?.openFilePreview?.({
    ...
    spaceId: agentId || props.spaceId,
    roomType: 'bot_person_chat',
  })
}
  ...
  <div v-if="attachments.length" class="msg-attachments" :class="[`role-${role}`]">
    ...
    >
      ...
      <div class="card-actions">
        ...
        >
          <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="#2F3547" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M2 9.5v2.5a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V9.5M5 9.5H2m3 0 1 1.5h4l1-1.5m0 0h3M8 2v5.5M6 5.5 8 7.5l2-2"/></svg>
        </button>
        <button
          type="button"
          class="save-to-lib-btn"
          title="下载到本地"
          :disabled="!canDownload(img)"
          @click.stop="smartDownload(img)"
        >
