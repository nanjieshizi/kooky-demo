...
import { fetchInstalledSkills } from '../services/skillApi'
  ...
  try {
    ...
    if (!hasKeyword) {
      ...
      const [items, agents] = await Promise.all([
        fetchInstalledSkills(options),
        props.agentId ? threadApi.listMyAgents({ stateful: true }) : Promise.resolve(null),
      ])
      // 已安装接口返回的技能默认标记为已安装
...
// agentId 变化时，若 popover 已打开则刷新列表；未打开则等下次打开时再加载
watch(
  () => props.agentId,
  () => {
    if (popoverVisible.value) loadSkills()
  }
)</script>
