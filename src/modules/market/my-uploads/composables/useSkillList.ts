...
import type { SkillItem, StatusFilter } from '../types'

/**
 * Skill 列表管理组合式函数（支持下拉加载）
 * @param searchKeyword 与工具栏搜索框同步，作为 GET my/skills 的 `search` 查询参数
 * @param statusFilter 可选的状态筛选
 */
export function useSkillList(searchKeyword: Ref<string>, statusFilter?: Ref<StatusFilter>) {
  ...
  // 监听状态筛选变化，重新加载列表（仅当 statusFilter 传入时）
  if (statusFilter) {
    watch(statusFilter, () => {
      page.value = 1
      list.value = []
