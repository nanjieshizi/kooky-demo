<template>
  <Teleport to="body">
    <div v-if="visible" class="project-backdrop" role="presentation" @mousedown.self="requestClose">
      <section class="project-modal" role="dialog" aria-modal="true" aria-label="创建协作背景">
        ...
        <div class="project-content">
          <main class="project-form">
            <div class="form-section">
              ...
              <label class="field"><span>协作背景</span><textarea v-model="form.background" placeholder="补充项目缘起、上下文或已知限制（可选）" rows="3" @input="dirty = true" /></label>
            </div>
          ...
          <aside class="check-sidebar">
            <div class="check-header"><h3>发布前检查</h3><p>协作背景发布后将成为项目公共上下文</p></div>
            ...
            <div class="freeze-card"><h4>发布后会发生什么？</h4><ul><li>生成协作背景 v1</li><li>成为任务创建时的可选上下文</li><li>新任务可引用目标与约束</li><li>后续修改会生成新版本</li></ul></div>
          </aside>
        </div>

        <footer class="project-footer"><span>草稿自动保存于刚刚 · 项目成员可见</span><div><button class="secondary-button" @click="saveDraft">保存草稿</button><button class="primary-button" @click="createBase">创建协作背景</button></div></footer>
...
function saveDraft() { dirty.value = false; showToast('协作背景草稿已保存') }
function createBase() { if (!validate()) { showToast('请先补充必填信息'); return }; dirty.value = false; emit('created', { ...form, version: 'v1' }); showToast('协作背景已创建，当前版本为 v1'); window.setTimeout(() => emit('update:visible', false), 700) }
function requestClose() { if (dirty.value && !window.confirm('协作背景尚未保存，确定退出吗？')) return; emit('update:visible', false) }
</script>
