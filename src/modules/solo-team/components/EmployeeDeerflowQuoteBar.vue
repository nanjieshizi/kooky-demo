...
const previewText = computed(() => {
  const text = stripBasicMarkdown(props.quotingMessage.content || '')
  return text.length > 60 ? text.slice(0, 60) + '...' : text
})

const fullText = computed(() => stripBasicMarkdown(props.quotingMessage.content || ''))
...
const displayName = computed(() => resolveAssistantDisplayName(props.quotingMessage, props.assistantName))
</script>
