...
export function useEmployeeDeerflowUserBubbleMarkdown(getMessage, options = {}) {
  ...
  function extractQuotesFromMarkdown(text) {
    ...
    for (const line of lines) {
      if (line.startsWith('> ')) currentQuote.push(line.slice(2))
      else if (line.startsWith('>')) currentQuote.push(line.slice(1))
      else {
    ...
    if (currentQuote.length > 0) quotes.push(currentQuote.join('\n').trim())
  ...
  watch(
    ...
    ([content, isStreaming, role, mentionedUserIds, skills]) => {
      ...
      if (isStreaming) renderTimer = setTimeout(() => runRender(content, mentionedUserIds, hasSkills), 80)
      else runRender(content, mentionedUserIds, hasSkills)
    },
    { immediate: true },
  )
