<template>
  <section class="personal-task-chat">
    ...
    <div class="task-chat-scroll">
      ...
      <article v-for="message in task.messages" :key="message.id" class="task-chat-message" :class="`is-${message.type}`">
        <span v-if="message.type === 'system'" class="task-system">{{ message.text }}</span>
        <template v-else-if="message.kind === 'file'"><b>团队助理</b><p>{{ message.text }}</p><div class="task-file-card"><span class="task-file-icon">M</span><div><strong>{{ message.file.name }}</strong><small>{{ message.file.type }} · {{ message.file.version }} · {{ message.file.status }}</small></div><button type="button">查看产物</button></div></template>
        <template v-else><b>{{ message.type === 'user' ? '我' : '团队助理' }}</b><p>{{ message.text }}</p></template>
      </article>
