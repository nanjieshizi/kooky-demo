<template>
  ...
  >
    ...
    <div
      ...
      :style="{
        background: theme?.paneBg || 'rgba(0, 0, 0, 0.2)',
        '--close-color': theme?.paneCloseColor || 'rgba(255, 255, 255, 0.4)',
        '--close-hover-bg': theme?.paneCloseHoverBg || 'rgba(255, 80, 80, 0.3)',
        '--close-hover-color': theme?.paneCloseHoverColor || '#fff',
      }"
      @mousedown.stop="onHeaderMouseDown"
    >
      <slot name="pane-header" :leaf-id="node.id" :terminal-id="node.terminalId" />
      <button class="pane-close" @mousedown.stop @click.stop="emit('remove-leaf', node.id)" title="关闭">✕</button>
    </div>

    <!-- 叶子内容（终端） -->
    <div class="split-leaf-content">
      <slot name="leaf" :leaf-id="node.id" :terminal-id="node.terminalId" />
    </div>
  </div>
</template>
