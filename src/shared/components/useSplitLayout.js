...
export function useSplitLayout() {
  ...
  function init(terminalId) {
    ...
    layoutRoot.value = { type: 'leaf', id: leafId, terminalId }
  ...
  function addLeaf(targetLeafId, direction, newTerminalId) {
    const newLeafId = genId('leaf')
    const newLeaf = { type: 'leaf', id: newLeafId, terminalId: newTerminalId }
  ...
  function moveLeaf(fromLeafId, toLeafId, position) {
    ...
    const { terminalId } = fromResult.node
    removeLeaf(fromLeafId)
    // removeLeaf 后 toLeafId 仍在树中（只移除了 fromLeafId 对应的叶子）
    const newLeafId = addLeaf(toLeafId, position, terminalId)
    if (newLeafId === null) {
